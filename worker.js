...
<div class="login-wrap" id="loginWrap">
  <div class="login-box">
    ...
    <label style="display:flex; align-items:center; gap:8px; font-size:0.8rem; color:#8b8b9e; margin-bottom:14px; cursor:pointer;">
      <input type="checkbox" id="showPassChk" onclick="togglePassVisibility()"> Tampilkan Password
    </label>
...
function loadGames() {
  fetch('/api/games', { headers: { 'x-admin-password': adminPass } })
    ...
    .then(data => {
      ...
      filterSel.innerHTML = '<option value="">All Games</option>' + allGames.map(g => \`<option value="\${g.slug}">\${g.name}</option>\`).join('');
      genSel.innerHTML = allGames.map(g => \`<option value="\${g.slug}">\${g.name}</option>\`).join('');
      renderGamesList();
    });
}
...
function renderUsedCodesTable() {
  ...
  document.getElementById('usedCodesBody').innerHTML = used.map(c => \`<tr>
    <td class="game-cell">💗 \${c.game_name}</td>
    <td><div class="code-cell">\${c.code}</div></td>
    <td class="muted">\${deviceLabel(c)}</td>
    <td class="muted">\${c.activated_at || '—'}</td>
  </tr>\`).join('') || '<tr><td colspan="4" class="muted" style="text-align:center; padding:30px;">Belum ada kode terpakai</td></tr>';
}

function renderGamesList() {
  const el = document.getElementById('gamesList');
  el.innerHTML = allGames.map(g => {
    const codesForGame = allCodes.filter(c => c.game_slug === g.slug);
    const used = codesForGame.filter(c => c.status === 'ACTIVATED').length;
    ...
    .then(data => {
      container.innerHTML = \`
        <div class="content-editor">
          ...
          <textarea id="contentArea-\${slug}">\${JSON.stringify(data.content, null, 2)}</textarea>
